...
import { CheckCircle2, AlertTriangle, XCircle, ShieldCheck, Sparkles, RefreshCw } from 'lucide-react';
    ...
    <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 space-y-4">
      ...
      <div className="flex items-center justify-between pb-3 border-b border-slate-800">
        <div>
          ...
          <h3 className="font-extrabold text-white text-base mt-0.5">{activePreset.name}</h3>
        </div>

        <div className="text-right">
          <div className="text-xl font-extrabold text-emerald-400">{passPercentage}% Passed</div>
          <span className="text-[10px] text-slate-400">{passedChecks} of {totalChecks} checks compliant</span>
        </div>
      </div>
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="flex items-start gap-2.5">
                  ...
                  <div>
                    ...
                    {check.severity !== 'pass' && (
                      <p className="text-amber-300/90 text-xs mt-1.5 leading-relaxed bg-amber-500/10 p-2 rounded-lg border border-amber-500/20">
                ...
                {check.severity !== 'pass' && (
                  <button
                    ...
                    className="flex-shrink-0 text-[11px] font-bold text-emerald-400 hover:text-emerald-300 bg-emerald-500/10 hover:bg-emerald-500/20 border border-emerald-500/30 px-2.5 py-1 rounded-lg flex items-center gap-1 transition-colors"
                  >
